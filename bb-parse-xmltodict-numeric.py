...
cols=[i for i in test2.columns if i not in ['Turn','RulesEventBoardAction.Results.BoardActionResult.CoachChoices.ListDices']]
for col in cols:
    test2[col]=pd.to_numeric(test2[col])


# Solve for numeric first
...
test2['ActionDesc'] = test2['ActionType'].fillna("").apply(utilities.labelActions)
test2['RollDesc'] = test2['RollType'].fillna("").apply(utilities.labelRoll)
test2['ReRollStatus'] = test2['RollStatus'].fillna("").apply(utilities.labelRollStatus)
...
team_merge[['PlayerId','IdPlayerTypes']] = team_merge[['PlayerId','IdPlayerTypes']].apply(pd.to_numeric)
